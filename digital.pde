...
void graphtitle(){
  String title= "Digital Data";
  pushStyle();
  fill(20);
  textSize(24);
  textAlign(CENTER, BOTTOM);
  text(title, width/2.0, 60.0);
  popStyle();
}
...
class PolarGraph {
  ...
  float ylw = map(i,0,P.length,255,0);
  println(ylw);
      color strokeColor=color(ylw,ylw,0);  
  ...
  void creaConnect(float beta){                        //display hovered connections  
    ...
    //changing opacity of connectionsfill(#000000,cR75);                                //overwrite the text of the base, I dont like this much but I was lazy to change it
    text(P[i].getElemName(),R1,0);
    popMatrix(); 
    doConnections(i,color(cR75));  
    fill(000000);//draw connections
  }
  
  void creaConnect () {                 //display selected connections
    for (int i=0; i<P.length; i++){
      if (P[i].getOn()){                //if element is selected
        doConnections(i,#256760); 
        //do the job
      }
    }
  }
